...
void pwm_duty(u8 ratio)
{
	...
	{	
		...
		temp = (u16)(buff/100) - 1; 	
		EPR0 = (temp & 0x03ff ) >> 8;	//PWM1占空比重载寄存器高2位b<1:0>
		PRL0L = (temp & 0x00ff );		//PWM1占空比重载寄存器低字节b<7:0>
	}
}
void init_tim0(void)
{
...
void pwm_init(u32 Hz)
{
	...
	u16 buff_hz = 0;
	//PWMFS = 1;	//PWM高低频模式选择位	1:PWM以高频模式输出		0:PWM以低频模式输出		
	PWMFS = 0;		//(注意:  高频模式下比低频模式下PWM的驱动能力要小)
	...
	T1PRLH = (pwm_cyc & 0x03ff) >> 8;		//PWM0/1周期寄存器的高2位	b<1:0>
	T1PRLL = (pwm_cyc & 0x00ff);			//PWM0/1周期寄存器的低字节	b<7:0>
	TMR1ON = 1;
}
...
void main(void)
{
	...
	while(HTS == 0)	asm("clrwdt");		//等待内部快时钟稳定,使用PICC编译器编译 ，嵌入的汇编指令 为 PIC指令
	...
	while(0)
	{
		...
		if(flag_2ms == true)
		{
			...
			PORTA ^= 0x20;				//RA5	2ms 取反 必须有，不然没输出
			Timer_ctr ++ ;
		if(Timer_ctr >= 100)	
		{
			Timer_ctr = 0;
